...
G_model.load_state_dict(torch.load(f'./model/{model}.pth'))
G_model.to(device)
G_model.eval()
...
def main():
    ...
    index = args.index
    nav_kind = args.fake
    result_save_path = f'./lab5a/result/{model}/{index}'
    ...
    # 标定时间
    cv2.putText(img, str(index).split('.')[0], (img_width-180, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2, cv2.LINE_AA)
