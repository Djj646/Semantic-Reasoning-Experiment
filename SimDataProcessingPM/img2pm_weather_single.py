...
index = '2'
pathes = []
pathes.append('E:/CARLADataset/Town01/1') # rain
pathes.append('E:/CARLADataset/Town04/1') # fog
pathes.append('E:/CARLADataset/Town04/2') # night
...
def main():
    result_save_path = f'./result/lab5'
    os.makedirs(f'{result_save_path}', exist_ok=True)
    
    print('>> Loading the png...')
    print('>> Generating pm and insert into imgs...')
    times = ['1687424140.772288', '1686473734.5619884', '1686475849.5969813']
    
    try:
        img_inserted = []
        for i in range(len(times)):
            img_path = f'{pathes[i]}/img'
            nav_path = f'{pathes[i]}/nav'
            # Reading the img and nav
            img = read_png(img_path, file_name=times[i])
            nav = read_png(nav_path, file_name=times[i])
        ...
        img_row1 = np.hstack((img_inserted[1], img_inserted[2], img_inserted[0]))
