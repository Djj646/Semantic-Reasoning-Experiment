...
def main():
    ...
    str_num = '0'+"{:.2f}".format(error_rate).split('.')[1]
    
    result_save_path = path+'/'+str(args.data)+f'/model_{model}_result_nav_dis{str_num}/'
    ...
    # video = cv2.VideoWriter(f'./video/{model}_blur_{str_num}.mp4', cv2.VideoWriter_fourcc(*'mp4v'), args.fps, (img_width,img_height))
    try:
        for file in tqdm(file_names[:args.num]):
            ...
            # nav施加干扰 easy: 0.03, moderate: 0.16, hard: 0.44
