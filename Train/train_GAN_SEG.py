...
from model_train_costmap import GeneratorUNet, GeneratorCNN ,Discriminator
from dataset_gan_path import CARLADataset, CARLADataset_Seg
from utils import write_params

random.seed(datetime.now())
torch.manual_seed(3832)
torch.cuda.manual_seed(383225)
...
train_dataset=CARLADataset_Seg(data_index=index_list,dataset_path='../datacollect/DATASET/CARLA/Segmentation/')
test_dataset=CARLADataset_Seg(data_index=[0],dataset_path='../datacollect/DATASET/CARLA/Segmentation/', eval_mode=True)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in bar:
        ...
        loss_S = criterion_seg(S_fake, label)+criterion_seg(S_fake_unpair, label)
        loss_S.backward()
        ...
        loss_pixel = 0.5 *( criterion_pixelwise(G_fake, label)+criterion_pixelwise(G_fake_unpair, label))
        # Total loss
