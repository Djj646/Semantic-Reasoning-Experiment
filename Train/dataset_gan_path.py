...
class CostMapDatasetReal(Dataset):
    def __init__(self, opt, data_index, dataset_path='../../REAL/data/', evalmode=False, 
    zero_point_probability=0.3,turning_probability=0.3,device=None,beta=(1,1.5)):
        ...
        self.zero_point_probability = zero_point_probability
        self.turning_probability = turning_probability
        self.beta=beta
        ...
        for i in self.data_index:
            self.t_list[i]=get_filelist(self.dataset_path, i)
            self.pos_list[i],self.yaw_list[i]=self.read_pos(i)
            self.vel_list[i]=self.read_vel(i)
            self.acc_list[i]=self.read_acc(i)
            self.turning_list[i]=self.find_turning(self.yaw_list[i])[0]
        # print(self.turning_list)
    ...
    def read_pos(self,index):
        ...
        with open(file_path, 'r') as file:
            ...
            for line in lines:
                ...
                x = float(sp_line[1])
                y = float(sp_line[2])
                yaw = float(sp_line[3]) * 180 / np.pi
                # xy_list.append(np.array([x,y]))
                # yaw_list.append(yaw)
                xy_dict[sp_line[0]] = np.array([x,y])
                yaw_dict[sp_line[0]] = yaw
            xy_list = [xy_dict[key] for key in self.t_list[index]]
            yaw_list = [yaw_dict[key] for key in self.t_list[index]]
    ...
    def read_vel(self,index):
        ...
        with open(file_path, 'r') as file:
            ...
            for line in lines:
                ...
                vx = float(sp_line[1])
                vy = float(sp_line[2])
                # vxy_list.append(np.array([vx,vy]))
                vxy_dict[sp_line[0]] = np.array([vx,vy])
            vxy_list = [vxy_dict[key] for key in self.t_list[index]]
        return vxy_list
    ...
    def read_acc(self,index):
        ...
        with open(file_path, 'r') as file:
            ...
            for line in lines:
                ...
                ax = float(sp_line[1])
                ay = float(sp_line[2])
                # axy_list.append(np.array([ax,ay]))
                axy_dict[sp_line[0]] = np.array([ax,ay])
            axy_list = [axy_dict[key] for key in self.t_list[index]]
        return axy_list
    
    def read_png(self,file_name:str,kind:str,index:int):
        item_path = self.dataset_path+str(index)+'/'+kind+'/'+file_name+'.png'
        item = cv2.imread(item_path)  
        return item

    def read_costmap(self,index):
        cm_list = []
        files = get_filelist(self.dataset_path, index)
        for file in files:
            cm = self.read_png(file,'ipm',index)[:,:,0]
            cm = self.transform(Image.fromarray(cm)).to(self.device)
            cm_list.append(cm)
        return cm_list

    def find_turning(self, yaw_list, yaw_threshold=0.5):
        turning_offset = 25 # 转弯开始对yaw角变化的提前量
        ...
        turning_indexes = np.where(diff_yaw >= yaw_threshold)[0] - turning_offset # np数组
    ...
    def __getitem__(self, index) -> dict:
        ...
        dataset_index = random.choice(self.data_index)
        t_list = self.t_list[dataset_index]
        pos_list = self.pos_list[dataset_index]
        yaw_list = self.yaw_list[dataset_index]
        vel_list = self.vel_list[dataset_index]
        acc_list = self.acc_list[dataset_index]
        turning_list = self.turning_list[dataset_index]
        # print(turning_list)
        # print(dataset_index)
        ...
        turning_condition = random.uniform(0.0, 1.0)
        # print(turning_condition,self.turning_probability,turning_condition<self.turning_probability)
        if turning_condition<self.turning_probability:
            # print('1')
            time_point_index = random.choice(turning_list)
            time_point_index += random.randint(-300,300)
        else:
            # print('2')
            time_point_index =  random.randint(0+self.img_step, len(t_list)-100)
        time_point_index = max(self.img_step,min(time_point_index,len(t_list)-100))
        # t1 = time.time()
        # print(time_point_index,turning_condition)
        # print(time_point_index in turning_list)
        # print(len(turning_list))
        # print(len(t_list))


        ## Collect Past Data
        for time_step in t_list[time_point_index+1-self.img_step:time_point_index+1]:
            # print(time_step)
            cm = self.read_png(time_step,'ipm',dataset_index)[:,:,0]
            cm = self.transform(Image.fromarray(cm))
            cms.append(cm)

        ## Collect Current Data
        vx,vy = vel_list[time_point_index]
        vx,vy = list(map(float, [vx,vy]))
        v_0 = (vx*vx+vy*vy)**0.5

        ## Collect Future Data
        if self.evalmode == False:
            zero_condition = random.uniform(0.0, 1.0) # MORE zero to select 
            if zero_condition < self.zero_point_probability:
                t = 0
            else:
                # t = random.uniform(0.0, 1.0)*self.max_t
                t = beta.rvs(self.beta[0], self.beta[1], size=1)[0]*self.max_t
                # t = random.uniform(0.0, 1.0)*1.0
                
            # print(zero_condition,t)
            t_list_float = list(map(float, t_list))
            current_time_point_index = time_point_index
            # print('len=',len(t_list_float),'index=',time_point_index)

            future_time_point_index = search(t_list_float[time_point_index:],t+t_list_float[time_point_index]) 
            # False or 0+,need plus current_time_point_index
            
            if future_time_point_index == -1:
                future_time_point_index = 0
            
            future_time_point_index = min(current_time_point_index+future_time_point_index,len(t_list_float)-1)

            t = t_list_float[future_time_point_index]-t_list_float[current_time_point_index]
            xy_ = pos_list[future_time_point_index]-pos_list[current_time_point_index]
            vxy_ = vel_list[future_time_point_index]
            axy_ = acc_list[future_time_point_index]
            yaw = yaw_list[current_time_point_index]
            ...
            t_range = np.linspace(0.0,1.0,33)*self.max_t
            t_list_float = list(map(float, t_list))
            for t_ in t_range:
                
                current_time_point_index = time_point_index
                future_time_point_index = search(t_list_float[time_point_index:],t_+t_list_float[time_point_index])
                ...
                if future_time_point_index == -1:
                    ...
                    t_ = t_list_float[future_time_point_index]-t_list_float[current_time_point_index]
                    xy_ = pos_list[future_time_point_index]-pos_list[current_time_point_index]
                    vxy_ = vel_list[future_time_point_index]
                    axy_ = acc_list[future_time_point_index]
            ...
            print('evalmode is Not True and is Not False,Strange Error!')
        # t3 = time.time()
        # print('t1:',t1-t0)
        # print('t2:',t2-t1)
        # print('t3:',t3-t2)
        ...
        ## Numpy(float,str,etc)-->Torch Tensor
        cms = torch.stack(cms)
        t = torch.FloatTensor([t])
        v_0 = torch.FloatTensor([v_0])
        xy = torch.FloatTensor(xy)
        vxy = torch.FloatTensor(vxy)
        axy = torch.FloatTensor(axy)

        return {'img': cms, 't': t, 'xy':xy, 'vxy':vxy, 'axy':axy, 'v_0':v_0}


class CostMapDataset_(Dataset):
    def __init__(self, opt, data_index, dataset_path='../datacollect/DATASET/CARLA/NoTrafficLightNoNPC/', evalmode=False, 
    zero_point_probability=0.3,turning_probability=0.3,device=None,beta=(1,1.5)):
        self.evalmode = evalmode
        self.data_index = data_index
        transforms_ = [ transforms.Resize((200, 400), Image.BICUBIC),
...
class CostMapDataset(Dataset):
    def __init__(self, data_index, opt, dataset_path='../datacollect/DATASET/CARLA/NoTrafficLightNoNPC/', evalmode=False):
        self.evalmode = evalmode
        self.data_index = data_index
        self.weights = []
    ...
    def __getitem__(self, index):
        ...
        if collision_flag and float(ts) >= float(collision_t):
            ...
            vx = np.cos(yaw)*_vx + np.sin(yaw)*_vy vxy = torch.FloatTensor([vx, vy])
        ...
        print('time:',finaltime-starttime)
        if self.evalmode:
...
class CARLADataset(Dataset):
    def __init__(self, data_index, dataset_path='../datacollect/DATASET/CARLA/NoTrafficLightNoNPC/', eval_mode=False):
        self.data_index = data_index
...
class CARLADataset_Seg(Dataset):
    def __init__(self, data_index, dataset_path='E:/CARLADataset/Town10HD', eval_mode=False):
        self.data_index = data_index
        self.eval_mode = eval_mode
        img_height = 128
        img_width = 256
    ...
    def find_turning(self, yaw_list, yaw_threshold=0.5):
        ...
        # 根据阈值，将偏航角变化率超过阈值的位置视为转弯点，下标值存入列表
        turning_indexes = np.where((diff_yaw < 300) & (diff_yaw >= yaw_threshold))[0] - turning_offset # np数组
    ...
    def __getitem__(self, index):
        ...
        if not self.eval_mode:
            ...
            fake_input_img = torch.cat((img, fake_nav), 0)
            return {'img_nav': input_img, 'label': label, 'fake_nav_with_img':fake_input_img, 'seg_nav':None}
        else:
            return {'img': img, 'nav': nav, 'fake_nav':fake_nav, 'label': label, 'file_name':file_name,'seg':1}
        ...
        while True:
            ...
            try:
                for i in range(-9,1):
                    ...
                    img = self.transform(img)
                    nav = self.transform(nav)#simply transform the same with img
                    imgs.append(img)
                    navs.append(nav)
...
# if __name__ == '__main__':
    ...
    # test_loader = DataLoader(CostMapDataset_(data_index=[10,20], opt=opt, dataset_path='../datacollect/DATASET/CARLA/NoTrafficLightNoNPC/',evalmode=True),
    ...
    #     _, img_time_steps, color, height, width = batch['img'].size()
    #     # print(batch['img'].size())
    #     img = batch['img'][0,-1,:,:,:].clone().data.numpy().squeeze()*127+128
    #     img = Image.fromarray(img).convert("RGB")
    #     draw =ImageDraw.Draw(img)

    #     _,time_step,_ = batch['xy'].size()
    #     batch['xy'] = batch['xy'].view(time_step,2)

    #     real_xy = batch['xy'].squeeze().data.numpy()
    #     real_x = real_xy[:,0]
    #     real_y = real_xy[:,1]
...
if __name__ == '__main__':
    ...
    test_loader = DataLoader(CARLADataset_Seg([2], dataset_path='E:/CARLADataset/Town04/', eval_mode=True),
                         batch_size=1, shuffle=False, num_workers=1, pin_memory=True,persistent_workers=True)
